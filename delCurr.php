...
<?php
  ...
  elseif($_SESSION['role']=="admin"){
	...
	$stmt = $db->stmt_init();
        if($stmt->prepare("SELECT job_id FROM proj_curr_work where job_id=? and cid=?") or die(mysqli_error($db))) {
			$stmt->bind_param("is", $jid, $user);
			$stmt->execute();
			...
			if($result->num_rows === 0) {
				...
				if ($db->query("DELETE FROM proj_curr_work WHERE cid='$user' AND job_id=$jid")){
					?>
					<script type = "text/javascript">
