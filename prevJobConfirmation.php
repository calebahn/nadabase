...
<?php
        ...
        if ($curr_date>=$pstart){
          ...
          VALUES ('$cid', '$job_id', '$pstart', '$pend')";
            if ($db->query($sql)){
                echo "<center><h3>The job has been marked as 'Previously Worked'!</h3><a class='btn btn-secondary btn-sm' href='GetJob.php?jid=$job_id' role='button'>Return to Job Page</a></center>";
            } else {
              echo "<center>
            ...
            echo "<center>
          <h3>Your start date is after your end date! Please go back and ensure you chose the correct start and end dates!</h3>
          <a class='btn btn-secondary btn-sm' href='prevJobForm.php' role='button'>Return</a>
        </center>";
          }
      }else {
          echo "<center>
